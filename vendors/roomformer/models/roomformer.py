...
except Exception:  # pragma: no cover - inference-only fallback
    custom_L1_loss = None
    MaskRasterizationLoss = None
...
def build(args, train=True):
    ...
    if custom_L1_loss is None or MaskRasterizationLoss is None:
        raise ImportError(
            "RoomFormer training dependencies are missing. "
            "Install detectron2/fvcore stack to enable training losses."
        )
